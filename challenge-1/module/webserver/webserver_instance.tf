...
resource "aws_security_group" "three-tier_webservers"{
  tags = {
    Name = "${var.ENVIRONMENT}-three-tier-webservers"
  }
...
resource "aws_autoscaling_group" "three-tier_webserver" {
  name                      = "three-tier_WebServers"
  max_size                  = 2
  min_size                  = 1
...
resource "aws_lb" "three-tier-load-balancer" {
  name               = "${var.ENVIRONMENT}-three-tier-lb"
  internal           = false
  load_balancer_type = "application"
  security_groups    = [aws_security_group.three-tier_webservers_alb.id]
...
resource "aws_lb_listener" "webserver_listner" {
  load_balancer_arn = aws_lb.three-tier-load-balancer.arn
  port              = "80"
  protocol          = "HTTP"
...
output "load_balancer_output" {
  value = aws_lb.three-tier-load-balancer.dns_name
}
